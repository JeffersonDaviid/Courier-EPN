...
public class Inventario {
    ...
    private void actualizarEstadoPaqueteBD(String id, String estado) {
        ...
        try {
            ...
            if (!(rs > 0)) {
                JOptionPane.showMessageDialog(null, "Error al guardar el nuevo estado", "Error",
    ...
    private void ingresarRegistro(Paquete paquete, int capacidadPaquete, String agencia) {
        // historial.registrarRegistro(new
        // Registro(getFecha(),getAgencia(paquete),paquete.getId()));
    ...
    // Metodo que clasifica un paquete en las diferentes listas (Para envio a
    // Sucursal - Para envio a domicilio - Para retiro de bodega)
    ...
    // Metodo para entregar el paquete al cliente desde la bodega/recepcion - La
    // llama el bodeguero
    public void entregarPaquete(String id) {
        ...
        // Se guarda la fecha de salida del paquete en el registro y se actualiza en la
        // base
    ...
    // Metodo que calcula la capcidad ocupada de los paquetes que ya han estado
    // almacenados
    ...
    // Metodo que calcula la fecha o dias que le faltan a un paquete, sin ser
    // reclamado
    ...
    // Metodo que consulta el historial de ingresos y salidas de los paquetes el dia
    // ejecutado
    ...
    // Metodo que retira los paquetes desde la lista correspondiente para cargarlos
    // a los camiones
    public Paquete retirarPaquete(String idPaquete) {
        ...
        // Se guarda la fecha de salida del paquete en el registro y actualizar en la
        // base
        historial.getRegistro(idPaquete).setFechaSalida(getFecha());
        historial.actualizarFechaDeSalida(idPaquete, getFecha());
        actualizarCapacidadOcupada(-clasificarCapacidad(paquete.getTamanio())); // Se actualiza la capacidad ocupada del
                                                                                // inventario
        return paquete;
    }
    ...
    private ArrayList<Paquete> getPaquetesListoParaEntregar() {
        ArrayList<Paquete> paquetes = new ArrayList<>();
        for (Paquete paquete : paquetesAlmacenados.values()) {
            if (paquete.getHistorialEstado().get(paquete.getHistorialEstado().size())
                    .getEstado() == "Listo para retiro de bodega") {
